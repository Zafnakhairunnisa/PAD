...
namespace App\Domains\Institutional\Http\Controllers\Frontend\PaudHI;
...
class PaudHIController
{
    ...
    public function __construct(
        PaudHiRecapitulationService $service
        ) {
        $this->service = $service;
        $this->categories = PaudHiCategory::select('id', 'name')->get();
    }
    ...
    {
        return inertia('Cluster2/PaudHI', [
            'recapitulation' => [
    ...
    private function generate_recapitulation_data()
    {
        ...
        $data = $locations->map(function ($location) use ($yearRanges) {
            $dataEveryYear = $yearRanges->map(function ($year) use ($location) {
                $dataByGender = $this->categories->map(function ($category) use ($year, $location) {
    ...
    private function generate_series_data($categories)
    {
         ...
         */
        return $this->categories->where('id', '!=', 3)->map(function ($category) use ($yearRange, $locations) {
