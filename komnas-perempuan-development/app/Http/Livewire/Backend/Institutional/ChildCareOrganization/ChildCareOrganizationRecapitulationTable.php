...
/**
 * Class ChildCareOrganizationRecapitulationTable.
 */
class ChildCareOrganizationRecapitulationTable extends DataTableComponent
{
    private ChildCareOrganizationRecapitulationService $service;
    ...
    public function boot(ChildCareOrganizationRecapitulationService $childForumRecapitulationService)
    {
        $any = new \stdClass();
        $any->id = '';
        $any->name = __('Semua');
    ...
    {
        return ChildCareOrganizationRecapitulation::with('location:id,name')
    ...
    {
        return [
            Column::make(__('Jumlah Organisasi'), 'value')
                ->sortable(),
                ...
                ->sortable(function (Builder $query, $direction) {
                    return $query->orderBy(
                        Location::select('name')
                            ->whereColumn('child_care_organization_recapitulations.location_id', 'locations.id'),
                        $direction
                    );
                }),
    ...
    {
        return 'backend.institutional.child_care_organization.recapitulation.includes.row';
    }
}
