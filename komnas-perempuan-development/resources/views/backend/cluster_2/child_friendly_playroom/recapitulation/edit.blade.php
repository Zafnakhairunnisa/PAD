...
@inject('category', '\App\Domains\Cluster2\Models\ChildFriendlyPlayroom\ChildFriendlyPlayroomCategory')

@section('title', __('Ubah Ruang Bermain Ramah Anak'))

<x-forms.patch wire:submit.prevent="submit">
    <x-backend.card>
        <x-slot name="header">
            @lang('Ubah Ruang Bermain Ramah Anak')
        </x-slot>
                ...
                <div class="col-md-10">
                    ...
                    >
                        @foreach ($category->get() as $category)
                            <option
                                @if ($category->id === old('category_id', $category_id)) selected @endif
                                value="{{ $category->id }}"
